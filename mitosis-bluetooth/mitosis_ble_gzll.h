...
// can't use NRF_LOG_ENABLED, insufficient memory! (s130 on 16K RAM)
#define SIMPLE_DEBUG
...
//#define APP_ERROR_CHECK(x) if (x!=NRF_SUCCESS) printf("ERROR in line %d\n", x)
...
static int m_keyboard_mode = MODE_BT;
...
uint8_t battery_level_get(void) {
	...
	printf("Sending BAS report: %d%% (%dmV)\n", percent, vbat_current_in_mv);
